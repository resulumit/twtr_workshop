...
- `tidyverse` ([Wickham et al., 2019](https://cran.r-project.org/web/packages/tidyverse/index.html)), for various tasks
...
- Collect the latest posts from one or more users
    ...
    - returns the most recent tweets, replies, and/or retweets

- Limited to 3,200 tweets
    - set the limit with the `n` argument
        - the default is `n = 100`
        - best accepts the multiples of 100
    - there is no `retryonratelimit` argument
...
Collect the most recent posts by Hadley Wickham
...
search_tweets(user = "hadleywickham",
              n = 200)
...
Collect the most recent posts by Hadley Wickham .yellow-h[and] David Robinson
...
get_timeline(user = `c("hadleywickham", "drob")`,
             n = 200)
...
---
## Data Collection &mdash; `get_timeline()` &mdash; Notes
...
- this does not seem to be true
    - this code returns .yellow-h[my] home-timeline, not Wickham's
...
11) Collect the most recent messages posted by three users 
- 100 from the first user, 200 from the second user, and 300 from the third user
- hint: see the function documentation on how to vary `n` per user

<br>

12) Collect as many tweets as possible from your own home-timeline 
...
- this returns a single column of user IDs, not usernames
   - can be combined with, for example, `lookup_users()` to see the associated usernames *etc*.
...
Get a list of users, .yellow-h[followed by] one or .yellow-h[more users]
- this returns a single column of user IDs, not usernames
   - can be combined with, for example, `lookup_users()` to see the associated usernames *etc*.
- limited with 15 users
   - use `retryonratelimit = TRUE` to surpass the limit
- limited also with 5,000 followers per user
   - `retryonratelimit = TRUE` does not help
...
- Collect posts *liked* by one or more users
    - by username or user IDs, with the `user` argument
    - returns the most recent tweets, replies, and/or retweets

- Limited to 3,000 posts
    - set the limit with the `n` argument
        - the default is `n = 200`
        - best accepts the multiples of 100
    - there is no `retryonratelimit` argument
...
get_favorites(user = "chrishanretty")
...
- Collect information on the retweets of one post
...
- Limited to 100 retweets
   - can be further limited with the `n` argument
...
16) Get a list of favorites by three users
- Compare and contrast the resulting data with the same information available on browser

<br>

17) Get a list of accounts retweeting a recent tweet of yours
...
- Note that
    ...
    - use the `trends_available()` argument to check availability
...
- Collect data on lists where a given user is a member
   - i.e., lists where a user appears

- Limited with 200 lists
...
- Note that
    ...
    - list IDs appear in browser search bar
...
- Alternatively, specify lists with two arguments together
   - `owner_user` and `slug`
   
```{r, eval=FALSE}

lists_statuses(owner_user = "tweetminster",
               slug = "UKMPs")
...
- Collect posts from the timeline of a list
   - specify the list with the `list_id` argument
       - e.g., with data from `lists_memberships()`
...
- Limited with 5,000 users
   
```{r, eval=FALSE}

lists_subscribers(list_id = "1405362")

lists_subscribers(owner_user = "tweetminster",
                  slug = "UKMPs")
...
- Limited with 1,000 subscriptions
   
```{r, eval=FALSE}

lists_subscriptions(user = "37999671")
...
- Collect tweets as they are posted real time
   ...
   - set the length of search with the `timeout` argument
      - can be set to infinity with `Inf`
...
- Data preparation on your project
   - 
...
Dis-aggregate the tweets, by splitting them into smaller units
...
Wickham, H., RStudio (2020b). [https://cran.r-project.org/web/packages/tidyverse/index.html](tidyverse: Easily Install and Load the 'Tidyverse'). R package, version 3.3.3.
